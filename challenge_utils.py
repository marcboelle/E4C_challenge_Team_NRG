...
def build_training_data(hourly_data_path):
    ...
    """
    time = hourly_data['datetime'].dt.tz_convert('UTC')

    # Supprimer les informations de fuseau horaire pour éviter les problèmes d'index
    time = time.dt.tz_localize(None)

    # Convertir en np.datetime64
    time = time.astype(np.datetime64)

    # Assurez-vous que l'index du DataFrame est correct
